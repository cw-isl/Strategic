...
const wrapText = (text, width, fontSize) => {
  const sanitized = String(text === undefined || text === null ? "" : text);
  if (!sanitized) return [""];
...
const computeItemRows = (row, currency) => {
  const baseItems = toArray(row.items);
    ...
    : [
        {
          ...
          origin:
            row.originCountry ||
            getNested(row, ["shipment", "originCountry"]),
          unit: row.unit || "EA",
...
export const createInvoicePdfBuffer = (row, options = {}) => {
  ...
  const importer = getNested(row, ["importer"]);
  const importerCompany = isObject(importer) ? importer.companyName : undefined;
  const importerAddress = isObject(importer) ? importer.address : undefined;
  const importerPhone = isObject(importer) ? importer.phone : undefined;
  const importerContactName = isObject(importer) ? importer.contactName : undefined;
  const importerEmail = isObject(importer) ? importer.email : undefined;
  const consigneeLines = [
    row.importCompanyName || importerCompany || "Consignee",
    row.importAddress || importerAddress || "",
    row.importCountry ? `Country: ${row.importCountry}` : "",
    row.importPhone || importerPhone
      ? `Tel: ${row.importPhone || importerPhone}`
      : "",
    row.importContactName || importerContactName
      ? `Attn. ${row.importContactName || importerContactName}`
      : "",
    row.importEmail || importerEmail
      ? `Email: ${row.importEmail || importerEmail}`
      : "",
  ].filter((line) => line);

  const notifyParty = getNested(row, ["notifyParty"]);
  const notifyCompany = isObject(notifyParty) ? notifyParty.companyName : undefined;
  const notifyAddress = isObject(notifyParty) ? notifyParty.address : undefined;
  const notifyCountry = isObject(notifyParty) ? notifyParty.country : undefined;
  const notifyPhone = isObject(notifyParty) ? notifyParty.phone : undefined;
  const notifyContactName = isObject(notifyParty) ? notifyParty.contactName : undefined;
  const notifyEmail = isObject(notifyParty) ? notifyParty.email : undefined;
  const notifyLines = [
    row.notifyCompanyName || notifyCompany || "Notify party",
    row.notifyAddress || notifyAddress || "",
    row.notifyCountry || notifyCountry
      ? `Country: ${row.notifyCountry || notifyCountry}`
      : "",
    row.notifyPhone || notifyPhone
      ? `Tel: ${row.notifyPhone || notifyPhone}`
      : "",
    row.notifyContactName || notifyContactName
      ? `Attn. ${row.notifyContactName || notifyContactName}`
      : "",
    row.notifyEmail || notifyEmail
      ? `Email: ${row.notifyEmail || notifyEmail}`
      : "",
  ].filter((line) => line);

  const itemWithCurrency = toArray(row.items).find(
    (item) => isObject(item) && item.currency
  );
  const currencySource = row.currency || (itemWithCurrency ? itemWithCurrency.currency : undefined) || "USD";
  const currency = String(currencySource).toUpperCase();
  ...
  const metaEntries = [
    ...
    },
    {
      ...
      value:
        row.incoterms ||
        getNested(row, ["shipment", "incoterms"]) ||
        row.transportMode ||
        "",
    },
    {
      ...
      value:
        row.paymentTerms ||
        getNested(row, ["shipment", "paymentTerms"]) ||
        "",
    },
    {
    ...
    },
    {
      ...
      value:
        ...
        getNested(row, ["shipment", "destinationCountry"]) ||
        row.country ||
        "",
    },
    {
  ...
  const portOfLoading = [
    getNested(row, ["shipment", "originCountry"]) || row.originCountry || "",
    formatDate(getNested(row, ["shipment", "dispatchDate"]) || row.dispatchDate),
  ]
    .filter((value) => value)
    .join(" · ");
  const finalDestination = [
    getNested(row, ["shipment", "destinationCountry"]) ||
      ...
      "",
    formatDate(
      getNested(row, ["shipment", "loadingDate"]) ||
        row.loadingDate ||
        row.departureDate
    ),
  ]
